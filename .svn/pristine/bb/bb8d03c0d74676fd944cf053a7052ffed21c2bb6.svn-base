...
function huzzaz_func( $atts ) {
    ...
    if( !$pro )
        ...
        if ( !$popoutlink )
            ...
            $gallery = '<a class="huzzazPopoutLink" onclick="window.open(\'https://huzzaz.com/proembed/' . $id . '?layout=popout' . '&search=' . $search . '\', \'newwindow\', \'scrollbars=1,width=1025, height=650\'); return false;" href="/' . $id . '">' . $linktext . '</a>';
    }
    return $gallery;
}
